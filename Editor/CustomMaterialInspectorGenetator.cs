...
{
    public class CustomMaterialInspectorGenerator : EditorWindow
    {
        ...
        {
            GetWindow<CustomMaterialInspectorGenerator>("PropGen");
        }
            ...
            using (new GUILayout.VerticalScope())
            {
                using (new GUILayout.VerticalScope())
                {
                    this.shaderFile = EditorGUILayout.ObjectField(
                        new GUIContent("Shader File"), this.shaderFile,
                        typeof(UnityEngine.Object),
                        false);

                    if(this.shaderFile != null) EditorGUILayout.LabelField("Path:" + Application.dataPath.Replace("Assets", string.Empty) + AssetDatabase.GetAssetPath(shaderFile));

                    GUILayout.Space(5);

                    this.templateFile = EditorGUILayout.ObjectField(
                        new GUIContent("Template File"), this.templateFile,
                        typeof(UnityEngine.Object),
                        false);

                    if(this.templateFile != null) EditorGUILayout.LabelField("Path:" + Application.dataPath.Replace("Assets", string.Empty) + AssetDatabase.GetAssetPath(templateFile));
